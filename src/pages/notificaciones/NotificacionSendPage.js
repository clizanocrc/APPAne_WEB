...
export const NotificacionSendPage = () => {
  ...
  const { para } = notificacionActiva;
  let fotoPara = defaultUser;
  if (para.images) {
    fotoPara = para.images;
  }
  ...
  const handleResponse = (e) => {
    ...
    const para = socketState.usuariosOnLine.filter(
      (usuario) => usuario.uid === notificacionActiva.para._id
    );
    const usuario = {
      ...
      >
        ...
        <div className={"ml-2 mr-2 mb-2"}>
          ...
          <div style={{ cursor: "pointer" }}>
            ...
            >
              <i className="fa fa-share bigicon"></i>
            </button>
          </div>
        </div>
      </div>
